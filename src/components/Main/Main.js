...
export default function Main(props) {
  ...
  useEffect(() => {
    ...
    if(page===0){
      setIsLoading(true)
      setData({...data,[url]:{...data[url],ranking:sampleGongyou,new:sampleGongyou}},setPage(1),setIsLoading(false))
    }
    if (isThreshold && !isLoading && !isMaxCount&&page===1) {
      setIsLoading(true);
      setData({...data,[url]:{...data[url],brand:sampleGongyou}},setIsThreshold(false,setIsLoading(false,setPage(2))))
    }
    if(isThreshold && !isLoading && !isMaxCount&&page>1){
      setIsLoading(true)
      setData({...data,[url]:{...data[url],suggestion:[...data[url].suggestion,...sampleGongyou]}},setIsLoading(false,setIsThreshold(false),setPage(page+1)))
    }
...
const HotTag = () => {
  ...
  return (
    <div>
      {hotTag.map(tag => (
        <Link key={tag.index} to={`/list/${tag.name}/1`}>
