...
export default function Main(props) {
  ...
  const [toplike, setToplike] = useState(null)
  ...
  const scrollEvent = () => {
    ...
    if (scrollTop > 2500) {
      setMoreData2(sampleMoredata)
    }
    if (scrollTop > 3300) {
      setMoreData3(sampleMoredata)
    }
  }
        ...
        <div className="loadmore-section">
          ...
          <div className="section1">
            {moreData1 ?
              ...
              : null}
            {moreData2 ?
              moreData2.slice(0, 6).map(data => (
                <ImgBox data={data} />
              ))
              : null}
            {moreData3 ?
              moreData3.slice(0, 6).map(data => (
                <ImgBox data={data} />
              ))
              : null}
          </div>
        ...
        hotuser.map((user) => (
          <div>
            <div className="user-image">
              <Link key={user.nick} to="/profile"><img src={user.image} alt="..." /></Link>
            </div>
